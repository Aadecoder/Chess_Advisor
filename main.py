...
model = YOLO(r"D:\PROGRAMMING\Projects\Chess_Advisor\weights\chess_detection.pt") # Make sure to have the correct path for your .pt file

print("The first iteration will take some time after that it should work pretty fast")

while(1):
    # Image of the chess Board
    path = input("Enter the Path of the Photo (or type 'q' to exit): ")
    if(path == 'q'):
        break
    path1 = path.removeprefix('"')
    path_final = path1.removesuffix('"')
    image = cv.imread(path_final)
